...
public class WarehouseOptimizeController {
    ...
    @PostMapping("/layout/auto-generate")
    public ResponseEntity<ApiResponse<WarehouseOptimizeModels.AutoGeneratedLayoutResponse>> autoGenerateLayout(
            @RequestBody(required = false) WarehouseOptimizeRequests.AutoGenerateLayoutRequest request) {
        return ResponseEntity.ok(ApiResponse.success(service.autoGenerateLayout(
                request == null ? null : request.getCustomerCode())));
    }

    @GetMapping("/location-hierarchy")
    public ResponseEntity<ApiResponse<WarehouseOptimizeModels.WarehouseLocationHierarchyResponse>> getLocationHierarchy(
            @RequestParam(required = false) String customerCode) {
        return ResponseEntity.ok(ApiResponse.success(service.getLocationHierarchy(customerCode)));
    }

    @PutMapping("/location-hierarchy")
    public ResponseEntity<ApiResponse<WarehouseOptimizeModels.WarehouseLocationHierarchyResponse>> saveLocationHierarchy(
            @Valid @RequestBody WarehouseOptimizeRequests.WarehouseLocationHierarchyRequest request) {
        return ResponseEntity.ok(ApiResponse.success(service.saveLocationHierarchy(request)));
    }

    @DeleteMapping("/location-hierarchy")
    public ResponseEntity<ApiResponse<WarehouseOptimizeModels.WarehouseLocationHierarchyResponse>> deleteLocationHierarchy(
            @RequestParam String scope,
            @RequestParam(required = false) String customerCode) {
        return ResponseEntity.ok(ApiResponse.success(service.deleteLocationHierarchy(scope, customerCode)));
    }
    ...
    public ResponseEntity<ApiResponse<WarehouseOptimizeModels.StockSyncSnapshot>> syncStock(
        ...
        Object layoutData = request == null ? null : request.getLayoutData();
        return ResponseEntity.ok(ApiResponse.success(service.syncStock(profileId, customerCode, layoutData)));
    }
    ...
    public ResponseEntity<ApiResponse<WarehouseOptimizeModels.StockDelta>> stockDelta(
        ...
        return ResponseEntity.ok(ApiResponse.success(service.loadStockDelta(
                ...
                request == null ? null : request.getLayoutData())));
    }

    @PostMapping("/viewer/working-status")
    public ResponseEntity<ApiResponse<WarehouseOptimizeModels.WorkingStatusSnapshotResponse>> workingStatusSnapshot(
            @RequestBody(required = false) WarehouseOptimizeRequests.WorkingStatusSnapshotRequest request) {
        String customerCode = request == null ? null : request.getCustomerCode();
        return ResponseEntity.ok(ApiResponse.success(service.loadWorkingStatusSnapshot(customerCode)));
    }
