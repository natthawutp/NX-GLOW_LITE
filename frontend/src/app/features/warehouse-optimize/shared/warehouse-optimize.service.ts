...
export class WarehouseOptimizeService {
  ...
  generateLayoutFromDb(customerCode?: string | null): Observable<AutoGeneratedLayoutResponse> {
    return this.api.post<AutoGeneratedLayoutResponse>(`${this.basePath}/layout/auto-generate`, {
      customerCode: customerCode ?? null
    }).pipe(map(response => response.data));
  }

  getLocationHierarchy(customerCode?: string | null): Observable<WarehouseLocationHierarchyResponse> {
    const params = customerCode ? { customerCode } : {};
    return this.api.get<WarehouseLocationHierarchyResponse>(`${this.basePath}/location-hierarchy`, params).pipe(map(response => response.data));
  }

  saveLocationHierarchy(payload: Record<string, unknown>): Observable<WarehouseLocationHierarchyResponse> {
    return this.api.put<WarehouseLocationHierarchyResponse>(`${this.basePath}/location-hierarchy`, payload).pipe(map(response => response.data));
  }

  deleteLocationHierarchy(scope: WarehouseLocationHierarchyScope, customerCode?: string | null): Observable<WarehouseLocationHierarchyResponse> {
    const query = customerCode
      ? `${this.basePath}/location-hierarchy?scope=${encodeURIComponent(scope)}&customerCode=${encodeURIComponent(customerCode)}`
      : `${this.basePath}/location-hierarchy?scope=${encodeURIComponent(scope)}`;
    return this.api.delete<WarehouseLocationHierarchyResponse>(query).pipe(map(response => response.data));
  }
  ...
  syncStock(profileId: number, customerCode: string, layoutData?: Record<string, unknown> | null): Observable<StockSyncSnapshot> {
    return this.api.post<StockSyncSnapshot>(`${this.basePath}/viewer/${profileId}/sync-stock`, {
      customerCode,
  ...
  loadWorkingStatusSnapshot(customerCode: string): Observable<WorkingStatusSnapshotResponse> {
    return this.api.post<WorkingStatusSnapshotResponse>(`${this.basePath}/viewer/working-status`, {
      customerCode
    }).pipe(map(response => response.data));
  }
}
